...
function parseCommitslog(commits, { owner, repo, source }, { issues: { owner: issuesOwner, repo: issuesRepo, source: issuesSource, ...issueResolution }, semver }) {
  // 输出解析后的参数
  console.log("commits:", commits);
  console.log("owner:", owner);
  console.log("repo:", repo);
  console.log("source:", source);
  console.log("issues owner:", issuesOwner);
  console.log("issues repo:", issuesRepo);
  console.log("issues source:", issuesSource);
  console.log("issue resolution:", issueResolution);
...
module.exports = class ReleaseNotes {
  ...
  constructor (context, { issueResolution, template, partials, helpers, semver } = {}) {
    ...
    const parseCommitss = parseCommits(context.commits, { owner, repo, source }, {
      issues: { owner, repo, source, ...issueResolution },
      semver: this._semver
    })
    parseCommitslog(context.commits, { owner, repo, source }, {
      issues: { owner, repo, source, ...issueResolution },
      semver: this._semver
    })
